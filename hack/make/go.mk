...
.PHONY: build/debug
build/debug: GO_GCFLAGS=${GO_GCFLAGS_DEBUG}
build/debug: GO_LDFLAGS=${GO_LDFLAGS_DEBUG}
build/debug: bin/$(APP)  ## Builds a dynamic executable with disable inline and optimization for debugger.
...
static: bin/$(APP)  ## Builds a static executable.
...
trace: GO_LDFLAGS+=${GO_LDFLAGS_STATIC}
trace: GO_BUILDTAGS+=${GO_BUILDTAGS_STATIC}
trace: GO_FLAGS+=${GO_INSTALLSUFFIX_STATIC}
...
coverage/ci:  ## Takes packages test coverage, and output coverage results to CI artifacts.
	...
	CGO_ENABLED=$(CGO_ENABLED) $(GO_TEST) -v $(strip $(GO_TEST_FLAGS)) $(strip $(GO_FLAGS)) -covermode=atomic -coverpkg=$(PKG)/... -coverprofile=${GO_TEST_COVERAGE_OUT} $(GO_PKGS) 2>&1 | tee /dev/stderr | go-junit-report -set-exit-code > /tmp/test-results/junit.xml
...
TODO:  ## Print the all of (TODO|BUG|XXX|FIXME|NOTE) in packages.
	@rg -t go -C 3 -e '(TODO|BUG|XXX|FIXME|NOTE)(\(.+\):|:)' --follow --hidden --glob='!vendor'
